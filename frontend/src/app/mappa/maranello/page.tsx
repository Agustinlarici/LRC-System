...
export default function MappaMaranelloPage() {
  const [rawSvg, setRawSvg] = useState('');
  const [stati,  setStati]  = useState<Record<string, ShapeState>>({});
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => { document.title = 'Maranello — STR'; }, []);
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-white flex flex-col">
      ...
      <div className="absolute top-4 left-4 z-10">
        <span className="text-gray-700 font-semibold text-lg">Maranello</span>
      </div>
      ...
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: svgHtml || '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#9ca3af">Caricamento...</div>',
        }}
      />

      {/* Legend bar */}
      <div className="absolute bottom-0 inset-x-0 flex items-center justify-center gap-6 bg-white/90 backdrop-blur border-t border-gray-100 py-3 px-4">
        {([
